...
void deledge(int s, int t)
{
	...
	for (long long int j = 0; j < du; j++)
	{
		...
		{
			v = g->GetNI(u.GetOutNId(j));
			ri[v.GetId()] = ri[v.GetId()] - (pi[s]/teleportProb*(1-teleportProb))/(du);
		}
		else
		{
			v = g->GetNI(u.GetOutNId(j));
			ri[v.GetId()] = ri[v.GetId()] + (pi[s]/teleportProb*(1.0-teleportProb))*(1.0/(du-1) - 1.0/(du));
		}			
	}
...
int main(int argc, char* argv[])
{
	...
	///把图上所有的边都保存到vector edges中
	long long int src_id,target_id;
	myedge temp_edge;
	std::vector<myedge> edges;
	for (TNGraph::TNodeI u = g->BegNI(); u != g->EndNI(); u++)
	{
		src_id = u.GetId();
		temp_edge.src = src_id;
		for ( long long int j = 0; j < u.GetOutDeg(); j++ )
		{
			TNGraph::TNodeI v = g->GetNI(u.GetOutNId(j));
			target_id = v.GetId();
			temp_edge.target = target_id;
	...
	{
		myedge e = edges[rand() % (edge_count+1)];
		if(g->IsNode(e.src) && g->IsNode(e.target) && g->IsEdge(e.src,e.target))
		{
			//g->DelEdge(e.src, e.target);
			deledge(e.src,e.target);
			cnt++;
		}
		//cout<<cnt<<endl;
	}
