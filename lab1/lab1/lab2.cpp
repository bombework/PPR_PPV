...
int main(int argc, char* argv[])
{
	...
	while(cnt< edge_count * atof(argv[2]) )
	{
		src = rand() % (maxId+1);
		target = rand() % (maxId+1);
	...
	std::vector<int> i=get_topk(pi,1000);
	std::vector<int> exact=get_topk(pexact,1000);
	...
	cout<<"交集个数="<<result.size()<<endl;


	if(TRACE){
	FILE *file;
	file = fopen("edge2.txt", "r");
	for(int i = 0; i<2;i++)
	{
		int s,t;
		fscanf(file,"%d %d", &s,&t);
